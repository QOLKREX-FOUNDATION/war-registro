...
export const MaterialTable = ({ data, handleGetRecords }) => {
  ...
  const columns = useMemo(() => [
    {
      accessorFn: (row) => {
        ...
        const timestamp =
          dateParse.getTime() / 1000 + dateParse.getTimezoneOffset() * 60;
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <DatePicker
            ...
            onChange={(date) => {
              ...
              const realStartDate =
                parseStartDate.getTime() / 1000 +
                parseStartDate.getTimezoneOffset() * 60;
              setStartDate(date);
    ...
    },
    {
      ...
      Cell: ({ cell }) => {
        ...
        return (
          <span className="text-sm font-medium">
            {ad && <>{ad[0]?.documentNumber ?? "No Disponible"}</>}
          </span>
        );
      },
    },
  ...
  const handleExportRows = (rows) => {
    ...
    const exportData = rows.map((row) => {
      ...
      const timestamp =
        dateParse.getTime() / 1000 + dateParse.getTimezoneOffset() * 60;
      const dateRegistring = dayjs
        .unix(timestamp)
        .format("DD/MM/YYYY", { utc: false });
      const date2Parse = new Date(row.original.date);
      const timestamp2 =
        date2Parse.getTime() / 1000 + date2Parse.getTimezoneOffset() * 60;
      const dateBirth = dayjs
        .unix(timestamp2)
        .format("DD/MM/YYYY", { utc: false });
      return {
        "Registro": dateRegistring,
        "Micro Chip": row.original.chip,
        "Mascota": row.original.name,
        "Tipo": row.original.type,
        "Sexo": row.original.gender,
        ...
        "Doc. de identidad": ad
          ...
          : "No Disponible",
        "Registrante": row.original.userName,
      };
      // return {
            ...
            renderRowActions={({ row, table }) => (
              <>
                <Box sx={{ display: "flex", flexWrap: "nowrap", gap: "8px" }}>
                  ...
                  <Tooltip title="Vacunas" placement="top">
                    <IconButton
                      color="primary"
                      // title='Vacunas'
                      onClick={() => {
