...
export const Animal = () => {
    ...
    const getAnimals = (limit = 10, offset = 0, search = '') => {

        if (search !== '') {
            firuApi.get(`/animals?limit=${ limit }&offset=${ offset }&search=${ search }`, {
                headers: {
        ...
        firuApi.get(`/animals?limit=${ limit }&offset=${ offset }`, {
            headers: {
                'x-token': web3.authToken
            },
        }).then((res) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                firuApi.delete(`/animals/${ id }`, {
                    ...
                    Swal.fire({
                        icon: "success",
                        title: "Animal eliminada correctamente",
    ...
    const handlePrev = () => {
        if (searchChange !== '') {
            getAnimals(10, 10 * (animalsData?.pagination?.prevPage - 1), searchChange)
            return;
        }
                ...
                <div className="flex items-center gap-2">
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            getAnimals(10, 10 * (e.target.value - 1))
                        }
                        }
                    >
                        {
