...
export const CodePhone = () => {
  ...
  const getCodes = (limit = 10, offset = 0, search = "") => {
    if (search !== "") {
      firuApi
      ...
      if (result.isConfirmed) {
        firuApi
          ...
          .then((res) => {
            ...
            Swal.fire({
              icon: "success",
              title: "Código de celular eliminada correctamente",
    ...
    watch("id")
      ? firuApi
          ...
          .then((res) => {
            ...
            Swal.fire({
              icon: "success",
              title: "Código de celular editada correctamente",
              showConfirmButton: false,
              timer: 1500,
            });
            setShow(false);
      ...
      : firuApi
          ...
          .then((res) => {
            ...
            Swal.fire({
              icon: "success",
              title: "Código de celular agregada correctamente",
              showConfirmButton: false,
              timer: 1500,
            });
            setShow(false);
          ...
          .catch((err) => {
            ...
            Swal.fire({
              icon: "error",
              title: "Error al agregar código de celular",
              showConfirmButton: false,
              timer: 1500,
            });
          });
  ...
  const handlePrev = () => {
    if (searchChange !== "") {
      getCodes(10, 10 * (codesData?.pagination?.prevPage - 1), searchChange);
      return;
    }
    getCodes(10, 10 * (codesData?.pagination?.prevPage - 1));
  };

  const handleNext = () => {
    if (
      codesData?.pagination?.currentPage === codesData?.pagination?.totalPages
    )
      return;
    if (searchChange !== "") {
      getCodes(10, 10 * (codesData?.pagination?.prevPage + 1), searchChange);
    ...
    <div className="flex flex-col items-center justify-center">
      <div className="w-full flex justify-between">
        <div className="flex flex-col gap-2">
          <span>Total de Código de celular</span>
          <span>{codesData.total}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={(e) => {
              ...
              getCodes(10, 10 * (e.target.value - 1));
            }}
          >
            {[...Array(codesData?.pagination?.totalPages)].map((_, i) => {
              return (
      ...
      <div className="w-full flex flex-col items-start justify-center">
        <table className="border mt-3 min-w-max w-full table-auto">
          ...
          <tbody className="text-gray-600 dark:text-white">
            {codes.map((code, i) => {
              return (
                <tr key={code.id} className="bg-gray-100 dark:bg-gray-700">
                  <td className="px-3 py-2">{i + 1}</td>
                  <td className="px-3 py-2">{code.name}</td>
                  <td className="px-3 py-2">{code.countryCode}</td>
                  <td className="px-3 py-2">{code.phoneCode}</td>
                  <td className="px-3 py-2">{code?.nationality}</td>
                  <td className="px-3 py-2">
                    <img
                      src={code?.image?.imageUrl}
                      alt={code.name}
                  ...
                  <td className="px-3 py-2 flex gap-4">
                    <button
                      onClick={() => {
                        ...
                        setValue("phoneCode", code.phoneCode);
                        setValue("countryCode", code.countryCode);
                        setValue("nationality", code.nationality);
                        setValue("image", code?.image?.imageUrl);
                        setShow(true);
                      }}
            ...
            {codes.length === 0 && (
              <tr className="bg-gray-100 dark:bg-gray-700">
                <td className="px-3 py-2 text-center" colSpan="8">
                  No hay Código de celular registradas
                </td>
              </tr>
            )}
          </tbody>
        ...
        {codesData && (
          <div className="w-full flex justify-center items-center gap-2 mt-3">
            ...
            <button
              disabled={codesData?.pagination?.currentPage === 1}
              onClick={() => {
            ...
            <span>
              {codesData?.pagination?.currentPage} de
              {codesData?.pagination?.totalPages}
            </span>
            <button
              disabled={
                codesData?.pagination?.currentPage ===
                codesData?.pagination?.totalPages
              }
              onClick={() => {
        ...
        <>
          ...
          ></div>
          {
            ...
            >
              ...
              {watch("id") ? (
                <h1 className="text-2xl font-bold">Editar Código de celular</h1>
              ) : (
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="phoneCode">Código de Celular</label>
                <input
                  type="text"
                  id="name"
                  className="input-text uppercase"
                  {...register("phoneCode", {
                    required: {
                      value: true,
                      message: "El código de celular es requerido",
                    },
                  })}
                  placeholder="Ej. +51"
                />
