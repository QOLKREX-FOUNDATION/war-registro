...
export const Step1 = ({
  ...
  useEffect(() => {
    ...
    if (codes) {
      const selectedCountryId = codes.find(
        ({ countryCode }) => countryCode === watch("country")
      )?.id;
      selectedCountryId && setValue("nationality", selectedCountryId);
    }
  }, [watch("country")]);

  console.log(watch("nationality"));

  useEffect(() => {
    if (
      codes &&
      (!watch("nationality") || watch("nationality") === undefined)
    ) {
      const selectedCountryId = codes.find(
        ({ countryCode }) => countryCode === watch("country")
      )?.id;
  ...
  }, [watch("nationality"), codes]);

  useEffect(() => {
    if (codes) {
      if (watch("documentType") !== "PASSPORT") {
        const selectedCountryId = codes.find(
          ({ countryCode }) => countryCode === watch("country")
        )?.id;
        selectedCountryId && setValue("nationality", selectedCountryId);
      ...
      <div className="flex justify-center w-full flex-wrap gap-2 max-w-5xl pt-3">
        <div className="w-full flex flex-col items-start justify-center gap-2 ">
          ...
          <FormSelect
            ...
            setValue={setValue}
            error={errors}
            isDisabled={watch("country") !== "" ? true : false}
            // value={watch("country")}
            onChange={(target) => {
              console.log(target.value);
              setValue("country", target.value);
