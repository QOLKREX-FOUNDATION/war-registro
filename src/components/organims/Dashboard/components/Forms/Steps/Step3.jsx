...
import { CustomAsyncSelectEntity } from "../CustomAsyncSelectEntity";
...
export const Step3 = ({
  ...
  const {
    ...
    handleProvinces,
    handleDistricts,
  } = useUbigeo(watch("country") || "NONE");

  console.log({ departments });

  const { entitiesRegister, entitiesRegisterById, loading } = useEntities(
    watch("district"),
    id,
    isEdit
  );
  ...
  return (
    <>
      <h2 className="pb-3 pt-3 text-xl font-semibold mt-3">LOCALIDAD</h2>
      <hr />
        ...
        {entitiesRegisterById.length > 0 ? (
          <>
            <div className="w-full flex flex-col items-start justify-center gap-2">
              <FormSelect
                label={
                  countryLabel[watch("country")].departmentsLabel ||
                  "Departamento"
                }
                property="department"
                options={
                ...
                value={departments.filter(
                  (values) =>
                    values.value === entitiesRegisterById[0]?.department.trim()
                )}
                values={register}
                watch={watch}
                setValue={setValue}
            ...
            <div className="w-full flex flex-col items-start justify-center gap-2">
              <FormSelect
                label={
                  countryLabel[watch("country")]?.provincesLabel || "Provincia"
                }
                property="province"
                options={[
                  {
                ...
                value={provinces.filter(
                  (values) =>
                    values.value.trim() ===
                    entitiesRegisterById[0]?.province.trim()
                )}
                values={register}
                watch={watch}
                setValue={setValue}
            ...
            <div className="w-full flex flex-col items-start justify-center gap-2">
              <FormSelect
                label={
                  countryLabel[watch("country")]?.districtsLabel || "Distrito"
                }
                property="district"
                options={[
                  {
                ...
                value={districts.filter((distric) => {
                  return (
                    distric.value === entitiesRegisterById[0]?.district.trim()
                  );
                })}
                values={register}
                watch={watch}
          ...
          <>
            <div className="w-full flex flex-col items-start justify-center gap-2">
              <FormSelect
                label={
                  countryLabel[formDataCache?.country]?.departmentsLabel ||
                  "Departamento"
                }
                property="department"
                options={departments}
            ...
            <div className="w-full flex flex-col items-start justify-center gap-2">
              <FormSelect
                label={
                  countryLabel[formDataCache?.country]?.provincesLabel ||
                  "Provincia"
                }
                property="province"
                options={provinces}
            ...
            <div className="w-full flex flex-col items-start justify-center gap-2">
              <FormSelect
                label={
                  countryLabel[formDataCache?.country]?.districtsLabel ||
                  "Distrito"
                }
                property="district"
                options={districts}
        ...
        <div className="w-full ">
          {address ? (
            ...
            <CustomAsyncSelect
              ...
              value={entitiesRegisterById.filter((entities) => {
            ...
            <>
              {pathName === "/formulario/solicitud-de-registro" ? (
                <CustomAsyncSelect
                  label="Elije la Entidad Registradora"
                  property="registerEntity"
        ...
        <div className="w-full flex gap-2 ">
          {entitiesRegisterById.length > 0 && (
            <>
              ...
              <span>
                {entitiesRegisterById[0]?.direction
                  ? entitiesRegisterById[0]?.direction
          ...
          <FormCheckbox
            ...
            options={[
              ...
              // },
              {
                value: "jurament3",
                label:
                  "Acepto compartir mi informaciòn en las busquedas de la plataforma.",
                required: {
                  value: false,
                  message: "Campo requerido",
                },
