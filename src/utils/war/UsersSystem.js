...
import ABIUSERSTATUS from "../../config/abi/war/UserSystemStatus.json";
...
export const Users = async (web3, account) => {
	try {
		...
		const contract = new web3.eth.Contract(
			ABIUSERSTATUS,
			CONTRACTS_WAR.UserSystemStatus
		);
...
export const getQuantity = async (web3, account) => {
	try {
		const address = toChecksumAddress(web3, account);
		const contract = new web3.eth.Contract(
			ABIUSER,
			CONTRACTS_WAR.UsersSystem
		);
		const response = await contract.methods.getQuantity(address).call();
		return response;
	} catch (e) {
		console.log(e);
	}
...
export const getUserER = async (web3, account) => {
	try {
		const address = toChecksumAddress(web3, account);
		const contract = new web3.eth.Contract(ABIUSER, CONTRACTS_WAR.UsersSystem);
		const response = await contract.methods.getUserER(address).call();
		return response;
	} catch (e) {
		console.log(e);
	}
...
export const getAccessAll = async (
	web3,
	account,
	access
) => {
...
export const deleteUser = async (
	...
	try {
		...
		const contract = new web3.eth.Contract(ABIUSER, CONTRACTS_WAR.UsersSystem);
		const response = await contract.methods
			.deleteUser(toChecksumAddress(web3, addressChecksum))
